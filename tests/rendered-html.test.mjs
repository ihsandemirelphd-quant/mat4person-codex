...
test("server-renders the evidence-first product", async () => {
  ...
  assert.match(sourceRegion, /data-source-candidate-count="4"/);
  assert.match(sourceRegion, /data-license-ready-count="1"/);
  assert.equal((sourceRegion.match(/data-source-candidate=/g) ?? []).length, 4);
  ...
  assert.match(html, /Four leads/);
  assert.match(html, /Zero borrowed claims/);
  assert.match(html, /A public link proves availability, not quotation rights/);
